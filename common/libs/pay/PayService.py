...
class PayService():
    ...
    def createOrder(self,member_id,items = None,params = None):
        ...
        try:
            #为了防止并发库存出问题了，使用悲观锁 select for update, 这里可以给大家演示下
            ...
            model_pay_order.status = -8 # 支付状态
            model_pay_order.express_status = -8 # 快递状态
            model_pay_order.express_address_id = express_address_id
            model_pay_order.express_info = json.dumps(  express_info ) # 快递信息
